...
class DailyMeanReversionTopLosers(LiveStrategy):
    ...
    def _buy_losers_at_open(self) -> None:
        ...
        alloc = cash / float(len(picked))

        for sym in picked:
            px = last_trade_price(self.alpaca, sym, fallback_timeframe="1Min")
            if px is None:
                continue
            qty = _floor_qty(alloc, float(px))
            if qty <= 0:
                continue
            # market buy
            self.alpaca.submit_order(symbol=sym, qty=qty, side="buy", type="market", time_in_force="day")
    ...
    def tick(self, ctx: StrategyTickContext) -> None:
        ...
        if self._already_traded_today(ctx.now_utc):
            return

        # Only act near market open to avoid re-ranking later
        # If we are within first 15 minutes after open, trade.
        open_ts = getattr(ctx.clock, "next_open", None)
        # Alpaca clock doesn't directly provide today's open; approximate using now and next_close.
        # MVP: trade on first tick of the day after market opens.
...
class MovingAverageTrend(LiveStrategy):
    ...
    def tick(self, ctx: StrategyTickContext) -> None:
        ...
        for sym in self.symbols:
            ...
            # stop/tp
            if qty > 0 and avg_entry and avg_entry > 0:
                if self.stop_loss and price <= avg_entry * (1.0 - self.stop_loss):
                    self.alpaca.submit_order(symbol=sym, qty=qty, side="sell", type="market", time_in_force="day")
                    continue
                if self.take_profit and price >= avg_entry * (1.0 + self.take_profit):
                    self.alpaca.submit_order(symbol=sym, qty=qty, side="sell", type="market", time_in_force="day")
                    continue

            if qty == 0 and float(s_ma) > float(l_ma):
                # allocate equal fraction of bot capital per symbol based on account cash
                acct = self.alpaca.get_account()
                cash = float(getattr(acct, "cash", 0.0))
                alloc = cash / max(1, len(self.symbols))
                buy_qty = _floor_qty(alloc, price)
                if buy_qty > 0:
                    self.alpaca.submit_order(symbol=sym, qty=buy_qty, side="buy", type="market", time_in_force="day")
